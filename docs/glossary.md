...
| **Generative model** | A model that learns the underlying distribution of the data — $P(x)$ — and can generate new samples (e.g. GPT, diffusion models, GANs). Contrast with **discriminative models** (logistic regression, SVM, random forest) that only learn the decision boundary $P(y|x)$. |
